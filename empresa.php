<!doctype html>
<html class="no-js" lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Casa Pieropan - Inicio</title>
    <link rel="stylesheet" href="r/css/normalize.css"/> 
    <link rel="stylesheet" href="r/css/foundation.css" />
    <link rel="stylesheet" href="r/css/slidebars.min.css" /> 
    <link rel="stylesheet" href="r/css/zigna.css" />    
    <script src="r/js/modernizr.js"></script>
  </head>
  <body id="empresa">
  <div id="sb-site" class="empresa">
  <header class="small-12">
  	<?php include 'html-scraps/nav.php'; ?>
  </header>  
  <section class="small-12">
      <article id="side" class="medium-5 large-3 column">
      <h1>TITULO</h1>
      <h2>SUBTITULO</h2>
      <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sit amet orci ultrices, pulvinar sem condimentum, cursus metus. Nunc quis tincidunt arcu. In eget cursus nisi, pretium rhoncus felis. Nunc adipiscing faucibus odio, at rhoncus odio pharetra accumsan. Sed posuere, ipsum eget elementum pharetra, massa ante laoreet risus, vel malesuada nulla enim ultricies quam. Etiam porttitor bibendum volutpat. Mauris eleifend lectus imperdiet fringilla placerat. Ut et malesuada eros. Sed gravida massa nisl, sed sagittis ligula varius vitae. Aliquam sed pretium tortor, et elementum elit. Morbi vitae ullamcorper nulla. Cras luctus neque iaculis tristique posuere. Nulla ullamcorper sem vitae sem laoreet ultrices.<br><br>

PROIN feugiat ultrices dignissim. Nullam sagittis neque non erat laoreet, at lobortis enim pulvinar. Nunc placerat felis vel aliquam eleifend. Praesent venenatis eu est at venenatis. Nam eget est ante. Vivamus mauris massa, lobortis ac nunc eu, sagittis lobortis lacus. Curabitur feugiat nec nisl vel accumsan. Duis ac lectus sed lacus adipiscing aliquam id ut nunc. Sed tempus arcu purus, eget congue ipsum euismod non. Maecenas massa neque, sagittis vel pulvinar ac, consequat vitae libero. Sed hendrerit magna urna, at aliquam sapien ultricies in. Donec vestibulum luctus interdum. Nam ac suscipit lorem. Phasellus varius lectus ut turpis dignissim mattis.</p>
      </article>
  </section>
  <footer>
  </footer>
  </div>
  
	<script src="r/js/vendor/jquery.js"></script>
    <script src="r/js/vendor/slidebars.min.js"></script>
    <script>
        (function($) {
            $(document).ready(function() {
                $.slidebars();
            });
        }) (jQuery);
    </script>  
 
  </body>
</html>
